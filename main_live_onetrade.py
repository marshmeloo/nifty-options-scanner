"""
Standalone live loop for the "one trade per day" candidate on NIFTY --
...
independent of main_live.py (Anchor) and main_live_sentinel.py
(Sentinel): own state, own journal, own log file, own decision log.

WHAT THIS IS: the EXACT SAME scan -> plan -> risk pipeline as Anchor,
with exactly one behavioural difference: config.MAX_NEW_TRADES_PER_DAY
is patched from its live default (999, effectively uncapped) to 1.
Nothing else is touched -- same conviction bar, same expiry-day rules,
same breakeven-arm exit, same position sizing (config.TOTAL_CAPITAL is
left at its real value; MAX_LOTS_PER_TRADE=1 already caps every trade
to one lot regardless, matching how research/one_trade_per_day_study.py
sized its own backtested trades -- see that module for why a SEPARATE,
smaller Rs1,00,000 figure is used only for DISPLAYING this tracker's
return%, never for sizing the actual position).

WHY THIS EXISTS: research/one_trade_per_day_study.py backtested this
exact rule (shadow.Policy(max_trades_per_day=1)) across the full
6-year reconstructed history and found it meaningfully more survivable
for a realistic Rs1,00,000 account than Anchor's full ~9-trades/day
volume -- max drawdown 20.7%/20.0% (vs Anchor's 44.8%/28.8% against
its own real Rs5,00,000 base), and a higher win rate (47.1% vs 41.4%),
plausibly because taking only the day's single best-scoring setup
selects higher quality on average than taking every setup that merely
clears the bar. This live process is what turns that backtest finding
into an actual forward-tracked answer, the same way Sentinel did for
the cluster-cap idea.

THE SAME BACKTEST ALSO FOUND: Anchor and Sentinel become mathematically
IDENTICAL under a 1-trade/day cap (Sentinel's cluster cap only ever
blocks a SECOND same-day position, which never exists here). So this
process intentionally does NOT carry Sentinel's cluster-cap config --
there would be nothing for it to do -- and is best read as "Anchor's
own logic, rate-limited to one shot per day" rather than a third
independent strategy family.

CONFIG OVERRIDE MECHANISM: same as main_live_sentinel.py -- `config`
is the SAME shared module every strategy file imports, patched with
this variant's ONE value (MAX_NEW_TRADES_PER_DAY) before any strategy
module is used. Safe ONLY because this runs as its own OS process;
scanner.py, plan_generator.py, and trade_tracker.py all read config
values live on every call, never take a module-level copy at import
time, so patching here can never leak into Anchor's or Sentinel's
separate processes.

DOES NOT RECORD ITS OWN SNAPSHOT ARCHIVE (config.RECORD_SNAPSHOTS =
False below), for the same reason Sentinel doesn't: this process sees
the identical NIFTY chain Anchor's process already archives to
logs/snapshots/ within the same few seconds.

STATUS: research/experimental, NOT part of the promoted Anchor/Sentinel
registry in STRATEGY_VERSIONS.md. Not yet added to
automation/start_trading.ps1 -- start/stop manually until there is
forward evidence worth deciding on.

Run:
  set DHAN_CLIENT_ID=...
  set DHAN_ACCESS_TOKEN=...
  python3 main_live_onetrade.py
"""
...
# --- The ONE behavioural difference from Anchor, patched before any
# strategy module is used. See research/one_trade_per_day_study.py for
# the backtest evidence behind capping at 1.
config.STRATEGY_NAME = "OneTradeDay"
config.STRATEGY_VERSION = "0.1-research"
config.MAX_NEW_TRADES_PER_DAY = 1
...
tt.OPEN_TRADES_PATH = tt.STATE_DIR / "open_trades_onetrade.json"
tt.JOURNAL_PATH = tt.LOG_DIR / "trade_journal_onetrade.jsonl"
tt.BREAKEVEN_SHADOW_JOURNAL_PATH = tt.LOG_DIR / "breakeven_shadow_journal_onetrade.jsonl"
tt.DELAY_PROBE_JOURNAL_PATH = tt.LOG_DIR / "execution_delay_journal_onetrade.jsonl"
decision_log.LOG_PATH = Path(__file__).parent / "logs" / "decision_log_onetrade.jsonl"
market_regime.BASELINE_PATH = market_regime.STATE_DIR / "regime_baseline_onetrade.json"
...
log_path = LOG_DIR / f"nifty_scan_onetrade_{datetime.now().strftime('%Y%m%d')}.log"
...
log = logging.getLogger("nifty_scanner_onetrade")
...
def run_once(expiry: str, state: dict):
    ...
    log.info(f"\n[{ts}] ({snapshot.source}) [ONETRADE] NIFTY spot {snapshot.spot}, VWAP-proxy {snapshot.vwap}, PCR {snapshot.pcr}")
    ...
    if state["opened_today"] >= config.MAX_NEW_TRADES_PER_DAY:
        ...
        if new_trade:
            log.info(
                f"  [NEW TRADE OPENED -- today's ONE] {new_trade['strike']} {new_trade['option_type']}  "
            ...
            log.info(f"    trades opened today: {state['opened_today']}/{config.MAX_NEW_TRADES_PER_DAY} -- done for today")
        elif results:
...
def check_open_trades_fast(state: dict, expiry: str):
    """Same orderflow-first/LTP-fallback fast check as main_live.py and
    main_live_sentinel.py -- see either's docstring for why this is
    cheap enough to run at FAST_CHECK_INTERVAL_SECONDS."""
    if not state["trades"]:
        return
...
def run_forever():
    ...
    log.info(f"[ONETRADE v0.1-research] Workspace: {ws.upper()}  |  git {git['branch']} @ {git['commit']}"
    ...
    log.info(f"[ONETRADE] Tracking expiry: {expiry}. Polling every {POLL_INTERVAL_SECONDS}s during market hours.")
    log.info(f"Max {config.MAX_NEW_TRADES_PER_DAY} new trade/day (the ONLY difference from Anchor -- "
             f"see research/one_trade_per_day_study.py), conviction bar {config.MIN_CONVICTION_SCORE_TO_TRACK}.")
    ...
    while True:
        ...
        if is_open:
            ...
            log.info(f"[{datetime.now().strftime('%H:%M:%S')}] [ONETRADE] Market closed, sleeping...")
            was_open_last_cycle = False
